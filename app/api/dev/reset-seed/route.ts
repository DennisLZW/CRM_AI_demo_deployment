...
export async function POST() {
  ...
  // Reuse the existing seed endpoint logic by calling it internally is possible,
  // but keeping it explicit avoids HTTP/self-call.
  const firstNames = [
    "Dennis",
    "Cinnis",
    "Alex",
  ...
  const customersToCreate = Array.from({ length: 50 }).map((_, i) => {
    ...
    const ownerId = pick([staff1.id, staff2.id]);

    return { name, email, company, status, notes, createdAt, ownerId };
  });
